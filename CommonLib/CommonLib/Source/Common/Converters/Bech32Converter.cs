...
namespace CommonLib.Source.Common.Converters
{
    ...
    {
        public static string ToBech32String(this byte[] data, string hrp)
        {
    ...
    {
        static readonly int[] GF1024_EXP = {
        ...
        static readonly int[] GF1024_LOG = {
        ...
        private static readonly byte[] Byteset;
        ...
        private static int[] LocateErrors(uint residue, int length)
        {
            if (residue == 0)
            ...
            if (l_s0 != -1 && l_s1 != -1 && l_s2 != -1 && (2 * l_s1 - l_s2 - l_s0 + 2046) % 1023 == 0)
            {
                ...
                return l_e1 % 33 != 0 ? Array.Empty<int>() : new[] { p1 };
            }
            ...
            for (var p1 = 0; p1 < length; p1++)
            {
                ...
                if (l_e1 % 33 != 0)
                    continue;
                return p1 < p2 ? new[] { p1, p2 } : new[] { p2, p1 };
            }
            return Array.Empty<int>();
        }

        public Bech32Encoder(string hrp) : this(hrp == null ? null : Encoders.ASCII.DecodeData(hrp.ToLowerInvariant()))
        {
        }

        public Bech32Encoder(byte[] hrp)
        {
            _hrp = hrp ?? throw new ArgumentNullException(nameof(hrp));
            var len = hrp.Length;
		...
		protected virtual bool VerifyChecksum(ReadOnlySpan<byte> data, int bechStringLen, out Bech32EncodingType encodingType, out int[] errorPosition)
        {
            ...
            else
            {
                ...
                var epos = Bech32EncodingType.All
                    .Select(e => LocateErrors(polymod ^ (uint)e.EncodingConstant, bechStringLen - 1))
                    .Where(e => e.Length != 0).MaxBy(e => e.Length);
                errorPosition = epos;
        ...
        {
			var values = _hrpExpand.Length + data.Length + 6 is var v and > 256 ? new byte[v] : stackalloc byte[_hrpExpand.Length + data.Length + 6];
            var valuesOffset = 0;
			_hrpExpand.AsSpan().CopyTo(values[valuesOffset..]);

            valuesOffset += _hrpExpand.Length;
            data.CopyTo(values[valuesOffset..]);
		...
		public virtual string EncodeData(ReadOnlySpan<byte> data, Bech32EncodingType encodingType)
        {
            ...
            if (data.ToArray().Any(b => b > 31))
                throw new FormatException("Data can only contain 5-bit integers, preprocess the data first or use `Encode(()` method instead");

			var combined = _hrp.Length + 1 + data.Length + 6 is var v && v > 256 ? new byte[v] : stackalloc byte[v];
			...
			data.CopyTo(combined[combinedOffset..]);
        ...
        public static Bech32Encoder ExtractEncoderFromString(string test)
        {
            ...
            return new Bech32Encoder(test[..i]);
        }
        ...
        protected virtual void CheckCase(string hrp)
        {
            if (hrp.Length is 0)
                return;
            var isLowercase = char.IsUpper(hrp[0]);
        ...
        private bool StrictLength { get; set; } = true;
        ...
        protected virtual byte[] DecodeDataCore(string encoded, out Bech32EncodingType encodingType)
        {
			...
			var data = encoded.Length - pos - 1 is var v2 && v2 > 256 ? new byte[v2] : stackalloc byte[v2];
            ...
            if (VerifyChecksum(data, encoded.Length, out encodingType, out var error)) 
		...
		public string Encode(byte witnessVersion, byte[] data)
		{
			if (data is null)
				throw new ArgumentNullException(nameof(data));
			return Encode(witnessVersion, data.AsSpan());
		}

		public string Encode(byte witnessVersion, ReadOnlySpan<byte> data)
		{
			if (witnessVersion > 16)
				throw new ArgumentOutOfRangeException(nameof(witnessVersion), "Invalid decoded witnessVerion, should <= 0 and > 16");
			var bits = ConvertBits(data, 8, 5);
			var fiveBitData = 1 + bits.Length is var v && v > 256 ? new byte[v] : stackalloc byte[v];
			fiveBitData[0] = witnessVersion;
			bits.AsSpan().CopyTo(fiveBitData[1..]);
			var ret = EncodeData(fiveBitData, witnessVersion == 0 ? Bech32EncodingType.BECH32 : Bech32EncodingType.BECH32M);
			return ret;
		}

        public string Encode(ReadOnlySpan<byte> data) => Encode(0, data);
    }
}
