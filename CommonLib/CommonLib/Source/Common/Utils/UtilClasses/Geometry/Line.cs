...
namespace CommonLib.Source.Common.Utils.UtilClasses.Geometry
{
    public struct Line : IEquatable<Line>
    {
        ...
        public bool Contains(Point p)
        {
            ...
            return Math.Abs(Slope * p.X + Constant - p.Y) < TOLERANCE;
        }
        ...
        {
            if (Math.Abs(Slope - that.Slope) < TOLERANCE)
                return null;
        ...
        {
            return Math.Abs(Slope - that.Slope) > TOLERANCE;
        }
        ...
        {
            return Math.Abs(Slope) < TOLERANCE;
        }
        ...
        public bool IsPerpendicularTo(Line that)
        {
            ...
            if (that.IsVertical())
                return IsHorizontal();
            return Math.Abs(Slope * that.Slope - -1) < TOLERANCE;
        }
        ...
        public override string ToString()
        {
            ...
            if (Math.Abs(Slope - (-1)) < TOLERANCE) // Slope
                sb.Append("-x");
            else if (Math.Abs(Slope - 1) < TOLERANCE)
                sb.Append('x');
            else if (Math.Abs(Slope) > TOLERANCE)
                sb.Append(Slope).Append("*x");

            if (Math.Abs(Constant) > TOLERANCE) // Constant
            {
        ...
        public bool Equals(Line that)
        {
            return Math.Abs(Slope - that.Slope) < TOLERANCE && Math.Abs(Constant - that.Constant) < TOLERANCE;
        }
    }
}
